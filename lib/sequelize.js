...
class Sequelize {
  ...
  constructor(database, username, password, options) {
    ...
    console.log('============= sequelize timezone ================', this.options.timezone);
    ...
    switch (this.getDialect()) {
      ...
      case 'mssql':
        ...
        break;
      case 'mysql':
        Dialect = require('./dialects/mysql'); // 创建 mysql 连接
        break;
      case 'postgres':
  ...
  query(sql, options) {
    ...
    return Promise.try(() => {
      ...
      let bindParameters;
      console.log('[sequelize] >>>>><<<<<<<<<<<< query sql = ', sql);
      console.log('[sequelize] >>>>><<<<<<<<<<<< query sql = ', options.bind);
      console.log('[sequelize] >>>>><<<<<<<<<<<< query sql = ', this.options.dialect);
      if (options.bind) {
      ...
      console.log('[sequelize] >>>>> query sql = ', sql);
      console.log('[sequelize] >>>>> query bindParameters = ', bindParameters);
      return Promise.resolve(retry(() => Promise.try(() => {
        ...
        const query = new this.dialect.Query(connection, this, options);　//　实例化　Query　类，
        return this.runHooks('beforeQuery', options, query)
          .then(() => checkTransaction())
          .then(() => query.run(sql, bindParameters)) // sql 开始的地方
          .then(value => {
            console.log('[sequelize] query.run return =============  ', value);
            return value;
          })
          .finally(() => this.runHooks('afterQuery', options, query))
          .finally(() => {
            if (!options.transaction) {
              return this.connectionManager.releaseConnection(connection); // 如果没有开启事务，则查询操作结束之后释放连接
            }
          });
      }), retryOptions));
    });
